...
const Registration = () => {
  ...
  const [inputErrors, setInputErrors] = useState<IInputErrors>({ name: false, login: false, password: false, repeatPassword: false, user: false })
  const [inputErrorsPassword, setinputErrorsPassword] = useState<boolean>(false)

  const onReg = () => {

    const newErrors: IInputErrors = {
      name: name.length < 3,
    ...
    if (name.replace(/\s/g, "") &&
      ...
      repeatPassword.replace(/\s/g, "") &&
      password === repeatPassword &&
      !(newErrors.name || newErrors.login || newErrors.password || newErrors.repeatPassword)) {
    ...
    <LoginWrapper>
      <LoginBlock>
        <LoginContainer>
          ...
          <LoginInput maxLength={10} $error={inputErrors.login || inputErrors.user} placeholder='Логин' value={login} onChange={(e) => setLogin(e.target.value.replace(/\s/g, ""))} />
          <PasswordWrapper>
          ...
          {inputErrors.user && <ErrorSms>Пользователь с таким логином уже существует</ErrorSms>}
